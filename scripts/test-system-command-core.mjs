...
try {
  ...
  if (!core.policy.requiresApproval("system.execute")) {
    throw new Error("system.execute must require approval even when global approval mode is disabled");
  }

  const secretMarker = `approval-visible-${Date.now()}`;
  const invocation = core.execute("system.execute", {
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    pending = core.approvals.list().pending.find((item) => item.action === "system.execute");
    if (pending) break;
  ...
  core.setApprovalEnabled(false);
  if (!core.approvals.get(pending.id)) {
    throw new Error("Disabling global approval incorrectly cancelled mandatory system approval");
  }
  core.decideApproval(pending.id, "approve");
  ...
  const event = core.audit.list(20).find((item) => item.tool === "system.execute" && item.status === "success");
  ...
  console.log("PASS: system.execute always pauses for one-time local approval");
