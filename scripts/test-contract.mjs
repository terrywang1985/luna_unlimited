...
const client = new Client({ name: "luna-v070-contract", version: "0.7.0" });
...
try {
  ...
  if (byName.has("read_text_file") || byName.has("write_text_file") || byName.has("clone_repository")) {
    throw new Error("Legacy flat MCP tools must not be exposed in v0.7");
  }
  ...
  if (capabilities?.server?.version !== "0.7.0" || Object.keys(capabilities.tools || {}).length !== 13) {
    throw new Error("Capability catalog did not describe the v0.7 compact tools");
  }
  ...
  console.log("PASS: v0.7 exposes exactly 13 compact domain tools and no legacy flat tools");
